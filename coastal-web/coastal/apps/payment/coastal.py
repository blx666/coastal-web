...
def charge(rental_order, user):
    if user.coastalbucket.balance < rental_order.total_amount_usd:
        return False
    ...
    user.coastalbucket.balance -= rental_order.total_amount_usd
    user.coastalbucket.save()

    PaymentEvent.objects.create(
        order=rental_order,
        payment_type='coastal',
        amount=rental_order.total_amount_usd,
        currency='USD',
    )

    rental_order.coastal_dollar = rental_order.total_amount_usd
